...
void send_reading(float reading, float target, float time,
		AsyncWebSocketClient * client, bool reset) {
...
void setupController() {
	...
	// report messages
	controller->onMessage([](const char * msg) {
		StaticJsonBuffer<200> jsonBuffer;
		JsonObject &root = jsonBuffer.createObject();
		root["message"] = msg;
		textThem(root);
	...
	controller->onHeater([](bool heater) {
	...
	// report readings
	controller->onReadingsReport(
			[](const std::vector<ControllerBase::Temperature_t>& readings, unsigned long elapsed) {
	...
	// report mode change
	controller->onMode(
			[](ControllerBase::MODE_t last, ControllerBase::MODE_t current) {
	...
	controller->onStage([](const char * stage, float target) {
...
void send_data(AsyncWebSocketClient * client) {
	...
	std::vector<ControllerBase::Temperature_t>::iterator I =
			controller->readings().begin();
	std::vector<ControllerBase::Temperature_t>::iterator end =
			controller->readings().end();
...
void setup() {
	...
	server.begin();
	controller = new ReflowController(config);
	setupController();
